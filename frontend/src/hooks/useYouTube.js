...
export const useYouTube = () => {
  ...
  useEffect(() => {
    ...
    if (code && state === 'youtube') {
      // Check if verifier exists before trying to use it
      const verifier = localStorage.getItem('youtube_verifier');
      if (!verifier) {
        console.error('No verifier found - redirecting back to connect page');
        // Optionally redirect back to connect page
